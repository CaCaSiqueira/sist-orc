...
from db.queries import (
    ...
    listar_categorias, listar_contas,
)
...
df_ativos = listar_parcelamentos(apenas_ativos=True,  user_id=uid)
df_todos  = listar_parcelamentos(apenas_ativos=False, user_id=uid)
...
df_vencer = parcelas_a_vencer_por_mes(meses=12, user_id=uid)
if not df_vencer.empty:
    st.subheader("Parcelas a Vencer (próximos 12 meses)")
    fig = px.bar(df_vencer, x="mes", y="total", labels={"mes": "Mês", "total": "R$"},
        ...
        with st.container(border=True):
            ...
            with col_acoes:
                novo_pagas = st.number_input(
                    "Parcelas pagas", min_value=0, max_value=total,
                    value=pagas, key=f"pagas_{p['id']}", step=1,
                )
                c1, c2 = st.columns(2)
