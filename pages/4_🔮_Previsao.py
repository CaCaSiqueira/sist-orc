...
for col, cor, nome in [("despesa", "#FF6B6B", "Despesa"), ("receita", "#00B894", "Receita")]:
    if col in pivot.columns:
        fig.add_trace(go.Bar(
            x=pivot["mes"], y=pivot[col],
            name=f"{nome} (histórico)",
            marker_color=cor,
            opacity=0.85,
        ))
    fig.add_trace(go.Bar(
        x=df_prev["mes"], y=df_prev[col],
        name=f"{nome} (previsão)",
        marker_color=cor,
        opacity=0.4,
...
fig2.add_trace(go.Scatter(
    x=todos_meses, y=todos_saldo,
    mode="lines+markers",
    line=dict(color="#6C5CE7", width=2),
    marker=dict(color=cores, size=8),
    name="Saldo",
))
...
df_prev_show["receita"] = df_prev_show["receita"].apply(fmt)
df_prev_show["despesa"] = df_prev_show["despesa"].apply(fmt)
df_prev_show["saldo"] = df_prev_show["saldo"].apply(fmt)
