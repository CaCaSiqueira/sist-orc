...
df = listar_orcamentos(mes_sel)
cats_df = listar_categorias(tipo="despesa")
...
with col_copy:
    with st.expander("📋 Copiar orçamento de outro mês"):
        ...
        if st.button("Copiar") and mes_origem and mes_origem != mes_sel:
            df_orig = listar_orcamentos(mes_origem)
            copiados = 0
...
if not df_com_limite.empty:
    ...
    fig.add_trace(go.Bar(
        name="Gasto",
        x=df_com_limite["categoria"],
        y=df_com_limite["gasto"],
        marker_color=cores_barra,
        text=df_com_limite["gasto"].apply(fmt),
        textposition="outside",
    ))
    fig.add_trace(go.Scatter(
        name="Limite",
        x=df_com_limite["categoria"],
        y=df_com_limite["limite"],
        mode="markers",
...
for _, row in df.iterrows():
    ...
    cor_prog = "#FF6B6B" if gasto > limite_atual and limite_atual > 0 else "#00B894"
