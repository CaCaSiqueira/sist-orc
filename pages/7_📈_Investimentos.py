...
df_ativos = listar_investimentos(apenas_ativos=True,  user_id=uid)
df_todos  = listar_investimentos(apenas_ativos=False, user_id=uid)
...
with tab_lista:
    if df_ativos.empty:
        ...
        df_resumo = resumo_investimentos(user_id=uid)
        col_pizza, col_barras = st.columns(2)
        with col_pizza:
            fig_pie = px.pie(
                df_resumo, values="total_atual", names="tipo",
                title="Distribuição por tipo (valor atual)", hole=0.45,
            ...
            with st.container(border=True):
                ...
                with c4:
                    with st.expander("Atualizar / Resgatar"):
                        novo_val  = st.number_input("Novo valor atual (R$)", min_value=0.0,
                                                    value=float(inv["valor_atual"]), step=10.0, format="%.2f",
                                                    key=f"val_{inv['id']}")
                ...
                if len(hist) > 1:
                    fig_hist = px.line(hist, x="data", y="valor", labels={"data": "", "valor": "R$"},
                                       markers=True, color_discrete_sequence=[cor])
...
with tab_resgatados:
    ...
    if df_res.empty:
        ...
        show = df_res[["nome", "tipo", "instituicao", "valor_investido", "valor_atual",
                       "rendimento", "rentabilidade_real", "data_aplicacao"]].copy()
